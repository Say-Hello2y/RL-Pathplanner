...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.grids.get_type(new_x, new_y) == 1:
            new_x, new_y = old_x, old_y
        # set reward to -1.4 with other four grid
        if action > 3:
            self.grids.set_reward(new_x, new_y, -1.4)
        self.reward = self.grids.get_reward(new_x, new_y)
        # restore reward  = -1
        self.grids.set_reward(new_x, new_y, -1)
        if new_x == 35 and new_y == 15:
            self.grids.set_reward(new_x, new_y, 10)
            self.reward = self.grids.get_reward(new_x, new_y)
    ...
    def render(self, mode='human', close=False):
        ...
        for i in range(len(self.path)):
            x, y = self._state_to_xy(self.path[i])
            if i==len(self.path)-1:
                next_x, next_y = (35, 15)
            else:
                next_x, next_y = self._state_to_xy(self.path[i+1])
            self.aline = rendering.Line(((x + 0.5) * u_size, (y + 0.5) * u_size), ((next_x + 0.5) * u_size, (next_y + 0.5) * u_size))
...
def RLworld():
    env = GridWorldEnv(n_width=40,  # 水平方向格子数量
                       n_height=20,  # 垂直方向格子数量
                       u_size=15,  # 可以根据喜好调整大小
                       default_reward=-1,  # 默认格子的即时奖励值
                       default_type=0)  # 默认的格子都是可以进入的
    env.action_space = spaces.Discrete(8)  # 设置行为空间数量
    ...
    env.ends = [(35, 15)]
    for i in range(10):
        env.types.append((i, 9, 1))
        env.types.append((39 - i, 9, 1))
    for i in range(8):
        env.types.append((9, 9 + i, 1))
        env.types.append((30, 9 + i, 1))
    for i in range(6):
        for j in range(6):
            env.types.append((17 + i, j, 1))
    env.rewards.append((35, 15, 10))
